    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          // 2. Admin panel button (always visible)
          IconButton(
            icon: const Icon(Icons.admin_panel_settings),
            tooltip: 'Admin Panel',
            onPressed: () {
              if (_userRole == 'admin') {
                // 3. Navigate to admin panel if admin
                Navigator.of(context).push(
                  MaterialPageRoute(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          return GridView.builder(
            ...
            itemCount: _userRole == 'admin' ? products.length + 1 : products.length,
            itemBuilder: (context, index) {
              // If admin, show admin panel card first
              if (_userRole == 'admin' && index == 0) {
                return Card(
                  elevation: 4,
                  child: InkWell(
                    onTap: () {
                    ...
                    child: const Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        Text('Admin Panel', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                );
              ...
              // Adjust index for products after admin card
              final productIndex = _userRole == 'admin' ? index - 1 : index;

              // 11. Get the data for one product
              final productDoc = products[productIndex];
              ...
              return ProductCard(
                ...
                onTap: () {
                  // 13. Navigate to the ProductDetailScreen
                  Navigator.of(context).push(
                    MaterialPageRoute(
